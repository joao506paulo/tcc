...
class _KnowledgeGraphPageState extends ConsumerState<KnowledgeGraphPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: _selectedNode != null && _selectedNode!.type != 'tag'
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: const Text('Filtrar Knowledge Graph'),
          content: Column(
  ...
  void _showStatsDialog() async {
    final controller = ref.read(knowledgeGraphControllerProvider.notifier);
    final stats = await controller.calculateStats();
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Estatísticas do Knowledge Graph'),
        content: SizedBox(
          width: 400,
          child: SingleChildScrollView(
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildStatRow('Total de Nós', stats.totalNodes.toString()),
                _buildStatRow('Total de Arestas', stats.totalEdges.toString()),
                _buildStatRow('Nós Inferidos', stats.inferredNodes.toString()),
                _buildStatRow('Arestas Inferidas', stats.inferredEdges.toString()),
                _buildStatRow('Nós Semânticos', stats.semanticNodes.toString()),
                _buildStatRow('Nós de Tags', stats.tagNodes.toString()),
                _buildStatRow('Densidade', stats.density.toStringAsFixed(3)),
                const SizedBox(height: 16),
                const Text('Nós por Classe:', style: TextStyle(fontWeight: FontWeight.bold)),
                ...stats.nodesByClass.entries.map((e) => 
                  Padding(
                ...
                const SizedBox(height: 16),
                const Text('Arestas por Predicado:', style: TextStyle(fontWeight: FontWeight.bold)),
                ...stats.edgesByPredicate.entries.map((e) => 
                  Padding(
                    padding: const EdgeInsets.only(left: 16, top: 4),
                    child: Text('${e.key}: ${e.value}'),
                  )
